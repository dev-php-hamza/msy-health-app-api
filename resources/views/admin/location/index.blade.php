		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel">
				<div class="x_title d-flex justify-content-between">
					<h2>All Locations</h2>
					<a class="btn add_btn" href="{{ route('locations.create') }}">Add</a>
				</div>
						...
						<table
							...
							<thead>
								<tr class="headings">
									...
									<th class="column-title">Health Centers</th>
							...
							<tbody>
								@forelse($locations as $location)
									<tr class="even pointer">
										...
										<td>{{ count($location->healthCenters) }}</td>
										<td class="last">
											<form action="{{ route('locations.destroy',$location->id) }}" method="POST">
												@csrf
												@method('DELETE')
												<a class="btn tableButtons view" href="{{ route('locations.show',$location->id) }}"><i class="fa fa-info-circle"></i></a>
	                                            <a class="btn tableButtons edit" href="{{ route('locations.edit',$location->id) }}"><i class="fa fa-edit"></i></a>
...
<script type="text/javascript">
	function getSearchResult(elem) {
		...
		if (typeof term != "undefined" && term != '') {
			window.location.href = base_url+'/admin/locations/search?term='+term;
		}
	}
</script>
@endsection