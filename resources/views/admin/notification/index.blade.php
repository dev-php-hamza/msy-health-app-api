		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel">
				<div class="x_title d-flex justify-content-between">
					...
					<a class="btn add_btn" href="{{ route('notifications.create') }}">Add</a>
				</div>
							...
							<tbody>
								@forelse($notifications as $notification)
									<tr class="even pointer">
										...
										<td><i class="fa fa-{{($notification->is_completed)?'check':'times'}}"></i></td>
										<td>{{ count($notification->users) }}</td>
										<td class="last">
											
											<form action="{{ route('notifications.destroy',$notification->id) }}" method="POST">
												@csrf
												@method('DELETE')
												<a class="btn tableButtons view" href="{{ route('notifications.show',$notification->id) }}"><i class="fa fa-info-circle"></i></a>
												{{-- <a class="btn tableButtons edit" href="{{ route('notifications.edit',$notification->id) }}"><i class="fa fa-edit"></i></a> --}}
...
<script type="text/javascript">
	function getSearchResult(elem) {
		...
		if (typeof term != "undefined" && term != '') {
			window.location.href = base_url+'/admin/notifications/search?term='+term;
		}
	}
</script>
@endsection