	...
	<div class="row">
		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel border-none">
				...
				<div class="x_content">
					<form class="form-horizontal form-label-left" method="post" action="{{ route('resources.update',$resource->id) }}" enctype="multipart/form-data">
						@csrf
						@method('PUT')	
						...
						class="required">*</span></label>
							<div class="col-md-6 col-sm-6 col-xs-6">
								<select class="form-control" name="lang" required>
									<option value="en" {{($resource->lang == 'en')?'selected':''}}>English</option>
									<option value="es" {{($resource->lang == 'es')?'selected':''}}>Spanish</option>
								</select>
							</div>
				        </div>
						<div class="form-group row">
                          ...
                          class="required">*</span></label>
							<div class="col-md-6 col-sm-6 col-xs-6">
								<input type="text" class="form-control @error('title') is-invalid @enderror" name="title" placeholder="Enter title" value="{{ old('title',$resource->title ) }}" required>
								@error('title')
						...
						<div class="form-group row">
                          ...
                          class="required">*</span></label>
							<div class="col-md-6 col-sm-6 col-xs-6">
								<input type="url" class="form-control @error('url') is-invalid @enderror" name="url" placeholder="https://beta-massyhealth.simplyintense.com/" value="{{ old('url',$resource->url ) }}" required>
								@error('url')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                          ...
                          class="required">*</span></label>
							<div class="col-md-6 col-sm-6 col-xs-6">
								<select class="form-control" name="for_employee" required>
									<option value="no" {{ ($resource->for_employee == 'no')?'selected':'' }}>No</option>
									<option value="yes" {{ ($resource->for_employee == 'yes')?'selected':'' }}>Yes</option>
									<option value="both" {{ ($resource->for_employee == 'both')?'selected':'' }}>Both</option>
								</select>
							</div>
						</div>
						<div class="form-group row">
                          ...
                          class="required">*</span></label>
							<div class="col-md-6 col-sm-6 col-xs-6">
								<textarea class="form-control" name="description" rows="5" placeholder="Description should be 240 Character " onkeyup="handleReverseCount(this)" maxlength="240" required>{{ old('description',$resource->description)}}</textarea>
								<div class="d-flex justify-content-end mt-2">
									<span class="badge badge-pill badge-primary reverse-count" id="count">{{ strlen($resource->description)}}/240</span>
								</div>
						...
						<div class="form-group row">
							...
							<div class="col-md-6 col-sm-6 col-xs-6">
								<input type="file" class="form-control" name="icon_file" value="{{ old('icon') }}">
							</div>
						</div>
						<!-- accordian -->
						<div class="accordion" id="newAccordian">
							...
							<label class="mt-4 ml-4">
							    <input type="checkbox" class="checkAll_inputs" {{($resource->check_all == 1)? 'checked': ''}}>
							    Check All
					        </label>
							<div class="card-body">
								...
								<div id="{{$country->territory_code}}" class="collapse" aria-labelledby="headingOne" data-parent="#accordionExample">
									<input type="checkbox" name="companyIds[]" value="{{$country->id.':0'}}" class="checkAll" id="checkAll" {{ in_array($country->id,$countryRes)? 'checked' : '' }}> Choose Country
									@foreach($country->companies as $company)
									<div class="checkbox card-body">
										<input type="checkbox" name="companyIds[]" value="{{$country->id.':'.$company->id}}" id="checkItem" {{ in_array($company->id, $companyRes)? 'checked' : '' }}> {{$company->name}}
									</div>
									@endforeach
								</div>
								@endforeach
