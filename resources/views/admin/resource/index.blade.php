		...
		<div class="col-md-12 col-sm-12">
			<div class="x_panel">
				<div class="x_title d-flex justify-content-between">
					<h2>All Resources</h2>
					<a class="btn add_btn" href="{{ route('resources.create') }}">Add</a>
				</div>
				...
				<div class="x_content">
					<div class="table-responsive">
							...
							<tbody>
								@forelse($resources as $resource)
									<tr class="even pointer">
										...
										<td>{{ $resource->url }}</td>
										<td>{{ ucfirst($resource->for_employee) }}</td>
										<td>{{ $resource->lang }}</td>
										<td>
										@foreach($resource->countries as $country)
											<p>{{$country->name}}</p>
										@endforeach
										</td>
										<td class="last">
											...
											<form action="{{ route('resources.destroy',$resource->id) }}" method="POST">
												@csrf
												@method('DELETE')
												<a class="btn tableButtons view" href="{{ route('resources.show',$resource->id) }}"><i class="fa fa-info-circle"></i></a>
												<a class="btn tableButtons edit" href="{{ route('resources.edit',$resource->id) }}"><i class="fa fa-edit"></i></a>
						...
						<div class="pull-right ml-3">
							{{ $resources->appends(\Request::except('_token'))->render() }}
						</div>
					</div>
				</div>
			</div>
